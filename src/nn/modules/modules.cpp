...
{
    throw std::runtime_error("Module not imp");
}
...
void Module::register_parameter(std::string name , std::shared_ptr<Tensor> param){
    ...
    {
        if(p.first == name){
            throw std::invalid_argument("Parameter "+ name+ " already register");
        }
    }
...
void Module::register_module(std::string name , std::shared_ptr<Module> module){
    ...
    for(const auto &m: _modules){
        if(m.first == name){
            throw std::invalid_argument("Module " + module +" already register");
        }
    }
